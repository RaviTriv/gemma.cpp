      ...
      &weights.vit_img_pos_embedding, [&](const auto* pos_emb_t) {
        for (size_t i = 0; i < num_real_patches; ++i) {
          ...
          for (size_t d = 0; d < model_dim; ++d) {
            act_row[d] +=
                static_cast<float>(x_emb[d]) + static_cast<float>(y_emb[d]);
          }
        }
      });
}
...
void PrefillVitGemma4(const ModelConfig& model_config,
  ...
  // 4. Final Norm (RMSNorm in Gemma 4? Yes, post_attention_norm etc are
  // RMSNorm). JAX has vit/transformer/norm_out/scale. In C++ weights.h we
  // registered: vit_encoder_norm_bias(finder_("enc_norm_bias")) (not used if
  // RMSNorm) vit_encoder_norm_scale(finder_("enc_norm_scale")) We should apply
  // final norm. Wait, does it use bias? Gemma 4 final norm is RMSNorm (no
  // bias). So vit_encoder_norm_bias will be empty. We can use PostNorm or
  // RMSNormInplaceBatched.
  // RMSNormInplaceBatched(weights.vit_encoder_norm_scale, activations.x,
  // env.ctx); Actually, vit.cc uses LayerNormBatched: Gemma 4 ViT has no final
  // encoder norm.
